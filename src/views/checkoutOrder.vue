...
const editingAddress = ref(false); // Are we in editing address mode?
...
const cityList = ref(Object.keys(taiwanLocations));

// Update district list based on selected city
const districts = computed(() => {
  return selectedCity.value ? taiwanLocations[selectedCity.value] : [];
});

// Calculate full address
const fullAddressComputed = computed(() => {
  return `${selectedCity.value}${selectedDistrict.value}${newAddress.value}`;
});

const store = useStore();
const router = useRouter();

const orderDetails = computed(() => store.state.orderData);

// watch([selectedCity, selectedDistrict], () => {
//   fullAddress.value = `${selectedCity.value}${selectedDistrict.value}`;
// });
...
const submitOrder = async () => {
  if (!selectedCity.value || !selectedDistrict.value || !newAddress.value.trim()) {
    alert('請填寫完整的地址信息。');
    return;
  }
  try {
    const payload = {
      memberID: orderDetails.value.memberID,
      productIDandQuentities: orderDetails.value.productIDandQuentities.map(item => ({
        productID: item.productID,
        quantity: item.quantity,
        price: item.price,
        name: item.name
      })),
      totalPrice: orderDetails.value.totalPrice
    };

    // 注意：將地址附加在 URL 上作為查詢參數
    const orderAddress = encodeURIComponent(`${selectedCity.value}${selectedDistrict.value}${newAddress.value}`);
    const response = await axios.post(`http://localhost:8080/customer/api/order/addOrder?orderAddress=${orderAddress}`, payload);

    if (response.status === 200) {
      router.push('/order');
      store.dispatch('removeOrderData');
    }
  } catch (error) {
